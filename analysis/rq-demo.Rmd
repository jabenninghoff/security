...
validate_risks <- local({
  validate_rules <- validator(
    ...
    # TODO: update after https://github.com/r-lib/lintr/issues/2790 is resolved
    risk_not_blank = nchar(risk) > 0, # nolint: nzchar_linter. validate doesn't recognize nzchar.
    desc_char = is.character(description),
    desc_not_na = !is.na(description),
    desc_not_blank = nchar(description) > 0 # nolint: nzchar_linter.
  )
...
validate_estimates <- local({
  validate_rules <- validator(
    ...
    expert_not_blank = nchar(expert) > 0, # nolint: nzchar_linter.
