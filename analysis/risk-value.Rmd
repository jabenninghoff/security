...
- [x] Does `meanlog + log(0.5)` really reduce the log-normal mean by 50% as the simulation suggests? Trial and error with hints from Google AI suggest that the correct approach is `meanlog - log(2)` (see [Appendix](#appendix)), which is equivalent to `meanlog + log(0.5)`.
- [ ] Use [Gordon–Loeb model](https://en.wikipedia.org/wiki/Gordon–Loeb_model) to determine optimal investment, see discussion on [SiRA Slack](https://sira.slack.com/archives/C016MJSJDGV/p1762546400851609)
...
[Information Risk Insights Study (IRIS)](https://www.cyentia.com/wp-content/uploads/2026/03/IRIS-2022_Cyentia.pdf).
...
reasonable value of $\lambda$ for a Poisson distribution that approximates these results without
increasing the number of breaches (it underestimates the number of multiple breaches but replicates
the number of single breaches):
...
```{r}
#| label: baseline_forecast
#| fig-alt: "A histogram showing number of losses by dollar value on a log10 scale. The curve is
#|          normally shaped."
...
The NPV of the risk reduction (return) is highly variable, and sometimes negative, because even
though we've reduced the overall risk, in a given 10 year period we might be unlucky and experience
a larger breach than in the baseline scenario. Since we can't plot negative numbers using a log
scale, we can examine the data using the cumulative distribution function (CDF). We limit the
x-axis to zoom in to the 1% to 99% quantiles:

```{r}
#| label: likelihood_ecdf
#| fig-alt: "An interactive plot showing the empirical CDF for the NPV of risk reduction. Most
...
- The median (50%) return is about $60,000
...
```{r}
#| label: impact_forecast
impact_forecast <- calc_risk("impact", lambda, meanlog - log(2), sdlog, runs = 100000 * 10) |>
  mutate(
    sim = ceiling(year / 10),
    year = year %% 10,
    year = if_else(year == 0, 10, year),
...
- The median (50%) return is about $1,000
...
[forecasting data breaches](https://web.archive.org/web/20251108062852/https://www.vivosecurity.com/download-isaca-philly-talk-may-2023)
...
```{r}
#| label: baseline_malware
#| fig-alt: "A histogram showing number of losses by dollar value."
lnorm_param(p05 = 200, p95 = 2000, p50 = 600)
...
Work out how to reduce impact by 50% through trial and error, with hints from Google AI:

```{r}
#| label: impact_test
base <- rlnorm(1e7, meanlog, sdlog)
reduced <- rlnorm(1e7, meanlog - log(2), sdlog)

base_sum <- summary(base)
reduced_sum <- summary(reduced)

base_sum
reduced_sum
base_sum / reduced_sum
```

`meanlog - log(2)` does appear to be the best way of cutting impact in "half"; the first quartile,
median, mean, and 3rd quartile are all reduced by approximately half. This is identical to the
original approach since `-log(2) == log(0.5)`, but is a bit clearer (divide by 2 vs multiply by
0.5).
