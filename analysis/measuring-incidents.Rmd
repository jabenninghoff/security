...
To determine the threshold of incidents needed to detect changes in duration, follow a similar
...
For step 3, use the geometric mean and
...
First, test at the 10% improvement level, with a sample size of 100, which is about 1 year's worth.
A single test is successful if the experimental group has a lower geometric mean and a positive
t-test (p < 0.05) on the log-transformed data.

```{r single_test}
single_test <- function(runs, improvement) {
  control <- rlnorm(runs, meanlog = meanlog, sdlog = sdlog)
  experiment <- rlnorm(runs, meanlog = meanlog, sdlog = sdlog) * (1 - improvement)
...
For most tests, the p-value is well above 0.05.
